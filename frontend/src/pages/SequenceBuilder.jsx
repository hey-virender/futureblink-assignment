...
const SequenceBuilder = () => {
  ...
  const handlePopoverAddNode = useCallback(
    (sourceNode, newNodeType) => {
      ...
      if (
        ...
        console.warn(
          `Invalid node addition request: from ${sourceNode.data.type} add ${newNodeType}`,
        );
        return; 
      }
  ...
  const canAddEmail = !!leadSourceNode && !emailNode;
  const canAddDelay = !!emailNode && !delayNode;
  
  const canDeleteNode =
    selectedNodeId === EMAIL_ID || selectedNodeId === DELAY_ID;
  ...
  return (
    <div
      className="h-[90vh] w-full relative bg-gray-100 p-4"
      ref={containerRef}
    >
